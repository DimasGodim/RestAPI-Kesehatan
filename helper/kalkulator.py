def HitungBMI(berat: float, tinggi: float, gender: bool):
    if gender.lower() == 1: # laki laki
        nilai_bmi = berat / (tinggi ** 2)
    elif gender.lower() == 0: # perempuan
...
def HitungBMR (gender: bool, berat_badan: float , tinggi_badan: float , usia: int): # jumlah kalori yang digunakan oleh tubuh posisi paling minimal
    if gender == 1: #pria
    ...
    if nilai_bmr < 1200:
        return {
            'nilai bmr': nilai_bmr,
            'kalimat saran': 'Untuk BMR yang sangat rendah, sangat penting untuk berkonsultasi dengan profesional kesehatan atau ahli gizi untuk mendapatkan panduan yang tepat. Fokus pada makanan bergizi tinggi untuk memenuhi kebutuhan nutrisi tanpa mengorbankan kesehatan',
            'plan diet': 'Dengan BMR yang sangat rendah, sangat penting untuk melakukan diet di bawah pengawasan profesional kesehatan. Fokus pada makanan bergizi tinggi dan pertimbangkan untuk memasukkan nutrisi tambahan agar kebutuhan tubuh terpenuhi'
        }
    elif 1200 <= nilai_bmr <= 1500:
        return {
            'nilai bmr': nilai_bmr,
            'kalimat saran': 'Dengan BMR yang rendah, disarankan untuk memilih makanan yang kaya nutrisi seperti sayuran hijau, protein tanpa lemak, dan buah-buahan. Penting untuk memastikan kecukupan nutrisi meskipun dalam batasan kalori yang lebih rendah',
            'plan diet': 'Meskipun BMR rendah, Anda masih dapat merencanakan diet dengan memilih makanan yang kaya nutrisi. Prioritaskan asupan protein, sayuran, dan buah-buahan, dan pertimbangkan untuk merencanakan makanan sepanjang hari untuk menjaga energi dan keseimbangan nutrisi'
        }
    elif 1500 <= nilai_bmr <= 1800:
        return {
            'nilai bmr': nilai_bmr,
            'kalimat saran': 'Bagi mereka dengan BMR sedang, pertahankan keseimbangan asupan protein, karbohidrat, dan lemak. Pilih sumber nutrisi yang seimbang dan pertimbangkan untuk memasukkan variasi makanan agar asupan nutrisi tetap optimal',
            'plan diet': 'Dengan BMR sedang, Anda memiliki fleksibilitas yang lebih besar dalam merencanakan diet. Pastikan untuk menjaga keseimbangan antara protein, karbohidrat, dan lemak. Sertakan makanan beragam untuk memastikan kebutuhan nutrisi terpenuhi'
        }
    elif nilai_bmr > 1800:
        return {
            'nilai bmr': nilai_bmr,
            'kalimat saran': 'Dengan BMR yang tinggi, fokus pada memenuhi kebutuhan kalori yang lebih besar dengan sumber protein berkualitas tinggi, lemak sehat, dan karbohidrat kompleks. Diversifikasi makanan untuk memastikan asupan nutrisi yang cukup',
            'plan diet': 'Dengan BMR tinggi, Anda dapat merencanakan diet yang mendukung aktivitas fisik dan kebutuhan kalori yang lebih besar. Pilih sumber protein berkualitas tinggi, lemak sehat, dan karbohidrat kompleks. Jangan lupa untuk tetap memerhatikan asupan nutrisi secara menyeluruh'
        }
